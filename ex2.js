...
app.get("/product/:category/:subcategory/:id",(req,res)=>{

  const {category,subcategory,id}=req.params;

 const findingCategory= storeData.find(item=>item.category===category);
...
const  findingSubcategory= findingCategory.subcategories.find(item=>item.subcategory===subcategory);



if(!findingSubcategory){
...
const  findingId= findingSubcategory.products.find(item=>item.id===Number(id));

if (findingId) {
  res.status(200).json(findingId);
  
}else{
  res.status(404).json({message:"Did not find the product."})
